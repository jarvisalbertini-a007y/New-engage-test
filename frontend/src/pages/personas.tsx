...
export default function Personas() {
  ...
  useEffect(() => {
    if (editingPersona) {
      setValue("name", editingPersona.name || "");
      setValue("description", editingPersona.description || "");
      setValue("targetTitles", editingPersona.targetTitles || []);
      setValue("industries", editingPersona.industries || []);
      setValue("companySizes", editingPersona.companySizes || []);
      setValue("valuePropositions", editingPersona.valuePropositions || {});
      setValue("toneGuidelines", editingPersona.toneGuidelines || {});
      setIsDialogOpen(true);
    }
  }, [editingPersona, setValue]);
  ...
  const updatePersonaMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => api.updatePersona(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/personas"] });
      handleCloseDialog();
      toast({
    ...
    onError: (error: any) => {
      ...
      toast({
        title: "Error",
        description: error?.message || "Failed to update persona. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const onSubmit = (data: PersonaFormData) => {
    const personaData = {
      ...
      targetTitles: data.targetTitles?.length > 0 ? data.targetTitles : null,
      industries: data.industries?.length > 0 ? data.industries : null,
      companySizes: data.companySizes?.length > 0 ? data.companySizes : null,
      valuePropositions: Object.keys(data.valuePropositions || {}).length > 0 ? data.valuePropositions : null,
      toneGuidelines: Object.keys(data.toneGuidelines || {}).length > 0 ? data.toneGuidelines : null,
    };

    if (editingPersona) {
      updatePersonaMutation.mutate({ id: editingPersona.id, data: personaData });
    } else {
  ...
  const isPending = createPersonaMutation.isPending || updatePersonaMutation.isPending;
      ...
      <header className="bg-card border-b border-border px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <Button onClick={handleOpenCreate} data-testid="button-create-persona">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <Textarea
                ...
                defaultValue={(watch("valuePropositions") as any)?.description || ""}
                onChange={(e) => {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {['Professional', 'Casual', 'Friendly', 'Urgent', 'Consultative', 'Educational', 'Conversational', 'Executive', 'Technical'].map((tone) => {
                  ...
                  const selectedTones = (currentGuidelines as any)?.tones || [];
                  const isChecked = selectedTones.includes(tone.toLowerCase());
              ...
              <div className="mt-3">
                ...
                <Textarea
                  ...
                  defaultValue={(watch("toneGuidelines") as any)?.additional || ""}
                  onChange={(e) => {
                    const currentGuidelines = watch("toneGuidelines") || {};
                    setValue("toneGuidelines", {
                      ...currentGuidelines,
          ...
          <CardContent>
            {isLoading ? (
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {(!persona.targetTitles || persona.targetTitles.length === 0) && (
                            <span className="text-xs text-muted-foreground">No titles specified</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {(!persona.industries || persona.industries.length === 0) && (
                            <span className="text-xs text-muted-foreground">No industries specified</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {(!persona.companySizes || persona.companySizes.length === 0) && (
                            <span className="text-xs text-muted-foreground">No sizes specified</span>
                          )}
                        </div>
                      </div>
              ...
              <div className="text-center py-12">
                ...
                <Button onClick={handleOpenCreate} data-testid="button-create-first-persona">
                  <Plus className="h-4 w-4 mr-2" />
                  Create Persona
                </Button>
              </div>
