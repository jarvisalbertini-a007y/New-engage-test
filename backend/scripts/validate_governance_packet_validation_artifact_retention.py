#!/usr/bin/env python3
"""Validate retention posture for governance packet validation artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate retention policy for governance packet validation artifacts."
    )
    parser.add_argument(
        "--artifact-dir",
...
def validate_retention(
    ...
    if invalid_contract_count > 0:
        errors.append(
            f"Found {invalid_contract_count} artifact(s) that failed governance packet validation artifact contract checks."
        )

    newest = records[0]
    newest_validated_at = newest.get("validatedAt")
    newest_path = newest.get("path")
    if not isinstance(newest_path, Path):
        errors.append("Unable to resolve newest governance packet validation artifact path.")
        return errors
    if newest_validated_at is None:
        errors.append(
            f"Newest governance packet validation artifact missing valid validatedAt: {newest_path}"
        )
        return errors

    age = datetime.now(timezone.utc) - newest_validated_at
    if age > timedelta(days=max_age_days):
        errors.append(
            f"Newest governance packet validation artifact {newest_path.name} is older than {max_age_days} days."
        )
...
def main() -> int:
    ...
    if errors:
        print("Governance packet validation artifact retention validation failed:")
        for error in errors:
            print(f"- {error}")
        return 1

    print(
        f"Governance packet validation artifact retention validation passed: {artifact_dir} "
