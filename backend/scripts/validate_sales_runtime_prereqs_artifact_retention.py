#!/usr/bin/env python3
"""Validate retention posture for runtime prerequisite artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate retention policy for runtime prerequisite artifacts."
    )
    parser.add_argument(
        "--artifact-dir",
    ...
    parser.add_argument(
        ...
        help="Runtime artifact filename prefix.",
    )
    parser.add_argument(
        "--min-count",
        type=int,
...
def validate_retention(
    ...
    if not isinstance(newest_path, Path):
        errors.append("Unable to resolve newest runtime prerequisite artifact path.")
        return errors
    if newest_generated_at is None:
        errors.append(f"Newest runtime prerequisite artifact missing valid generatedAt: {newest_path}")
        return errors

    age = datetime.now(timezone.utc) - newest_generated_at
    if age > timedelta(days=max_age_days):
        errors.append(
            f"Newest runtime prerequisite artifact {newest_path.name} is older than {max_age_days} days."
        )
...
def main() -> int:
    ...
    if errors:
        print("Runtime prerequisite artifact retention validation failed:")
        for error in errors:
            print(f"- {error}")
        return 1

    print(
        f"Runtime prerequisite artifact retention validation passed: {artifact_dir} "
