#!/usr/bin/env python3
"""Apply baseline metrics artifact cleanup only when policy gate allows."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply baseline metrics artifact cleanup only when policy allows."
    )
    parser.add_argument(
        "--artifact-dir",
...
def main() -> int:
    ...
    cleanup_module = _load_module("cleanup_baseline_metrics_artifacts.py")
    policy_module = _load_module("evaluate_baseline_metrics_artifact_cleanup_policy.py")
