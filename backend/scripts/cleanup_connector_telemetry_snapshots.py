#!/usr/bin/env python3
"""Cleanup stale connector telemetry snapshot artifacts with dry-run safety."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Cleanup stale connector telemetry snapshot files."
    )
    parser.add_argument(
        "--snapshot-dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Retain snapshots newer than this many days.",
    )
    parser.add_argument(
        "--keep-min-count",
        type=int,
...
def _parse_generated_at(snapshot_path: Path) -> datetime | None:
    try:
...
def _discover_snapshots(snapshot_dir: Path, prefix: str) -> List[dict]:
    records: List[dict] = []
    for path in sorted(snapshot_dir.glob(f"{prefix}*.json")):
...
def plan_cleanup(
    ...
    records = _discover_snapshots(snapshot_dir, prefix)
    if keep_days < 1:
...
def main() -> int:
    ...
    print(
        f"Telemetry snapshot cleanup {summary['mode']} completed: "
