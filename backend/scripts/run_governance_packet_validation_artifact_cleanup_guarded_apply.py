#!/usr/bin/env python3
"""Apply governance packet validation artifact cleanup only when policy gate allows."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Apply governance packet validation artifact cleanup only when policy allows."
        )
    )
    parser.add_argument(
        "--artifact-dir",
...
def main() -> int:
    ...
    cleanup_module = _load_module("cleanup_governance_packet_validation_artifacts.py")
