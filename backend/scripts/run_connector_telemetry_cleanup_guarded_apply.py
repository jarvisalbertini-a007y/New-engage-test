#!/usr/bin/env python3
"""Run telemetry snapshot cleanup apply-mode only when policy gates allow it."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Apply telemetry snapshot cleanup only when policy gate allows."
    )
    parser.add_argument(
        "--snapshot-dir",
...
def main() -> int:
    ...
    policy_module = _load_module("evaluate_connector_telemetry_cleanup_policy.py")
    cleanup_module = _load_module("cleanup_connector_telemetry_snapshots.py")
