#!/usr/bin/env python3
"""Evaluate policy gate for runtime prerequisite artifact cleanup apply-mode."""
...
ALLOW_APPLY_ENV_VAR = "RUNTIME_PREREQS_ARTIFACT_CLEANUP_ALLOW_APPLY"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate whether runtime prerequisite artifact cleanup may run in apply mode."
    )
    parser.add_argument(
        "--artifact-dir",
...
def _load_cleanup_module():
    script_path = Path(__file__).resolve().parent / "cleanup_sales_runtime_prereqs_artifacts.py"
...
def evaluate_policy_from_summary(
    ...
    *,
    ...
    if candidate_count <= 0:
        decision = "SKIP_APPLY"
        reason = "No stale runtime prerequisite artifacts detected."
        ...
        recommended_command = (
            ...
            "verify:baseline:runtime-prereqs:artifact:cleanup:apply:guarded"
        )
    else:
        decision = "ALLOW_APPLY"
        reason = "Stale runtime prerequisite artifact candidate count is within threshold."
        recommended_command = (
