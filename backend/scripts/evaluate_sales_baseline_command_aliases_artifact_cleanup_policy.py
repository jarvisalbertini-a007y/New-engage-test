#!/usr/bin/env python3
"""Evaluate policy gate for baseline command-alias artifact cleanup apply-mode."""
...
ALLOW_APPLY_ENV_VAR = "BASELINE_COMMAND_ALIASES_ARTIFACT_CLEANUP_ALLOW_APPLY"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate whether baseline command-alias artifact cleanup may run in apply mode."
    )
    parser.add_argument(
        "--artifact-dir",
...
def _load_cleanup_module():
    script_path = Path(__file__).resolve().parent / "cleanup_sales_baseline_command_aliases_artifacts.py"
...
def evaluate_policy_from_summary(
    ...
    *,
    ...
    if candidate_count <= 0:
        decision = "SKIP_APPLY"
        reason = "No stale baseline command-alias artifacts detected."
        ...
        recommended_command = (
            ...
            "verify:baseline:command-aliases:artifact:cleanup:apply:guarded"
        )
    else:
        decision = "ALLOW_APPLY"
        reason = "Stale baseline command-alias artifact candidate count is within threshold."
        recommended_command = (
