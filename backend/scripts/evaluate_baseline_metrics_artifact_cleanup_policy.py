#!/usr/bin/env python3
"""Evaluate policy gate for baseline metrics artifact cleanup apply-mode."""
...
ALLOW_APPLY_ENV_VAR = "BASELINE_METRICS_ARTIFACT_CLEANUP_ALLOW_APPLY"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate whether baseline metrics artifact cleanup may run in apply mode."
    )
    parser.add_argument(
        "--artifact-dir",
...
def _load_cleanup_module():
    script_path = Path(__file__).resolve().parent / "cleanup_baseline_metrics_artifacts.py"
...
def evaluate_policy_from_summary(
    ...
    *,
    ...
    if candidate_count <= 0:
        decision = "SKIP_APPLY"
        reason = "No stale baseline metrics artifacts detected."
        ...
        recommended_command = (
            "npm run verify:baseline:metrics:artifact:cleanup:dry-run -- --keep-days 7"
        )
        ...
        recommended_command = (
            ...
            "verify:baseline:metrics:artifact:cleanup:apply:guarded"
        )
    else:
        decision = "ALLOW_APPLY"
        reason = "Stale baseline metrics artifact candidate count is within threshold."
        recommended_command = (
