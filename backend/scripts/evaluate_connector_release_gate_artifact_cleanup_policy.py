#!/usr/bin/env python3
"""Evaluate policy gate for connector release-gate artifact cleanup apply-mode."""
...
ALLOW_APPLY_ENV_VAR = "RELEASE_GATE_ARTIFACT_CLEANUP_ALLOW_APPLY"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate whether connector release-gate artifact cleanup may run in apply mode."
        )
    )
    parser.add_argument(
        "--artifact-dir",
...
def _load_cleanup_module():
    script_path = Path(__file__).resolve().parent / "cleanup_connector_release_gate_artifacts.py"
...
def evaluate_policy_from_summary(
    ...
    *,
    ...
    if candidate_count <= 0:
        decision = "SKIP_APPLY"
        reason = "No stale connector release-gate artifacts detected."
        ...
        recommended_command = (
            "npm run verify:release-gate:artifact:cleanup:dry-run -- --keep-days 7"
        )
        ...
        recommended_command = (
            ...
            "verify:release-gate:artifact:cleanup:apply:guarded"
        )
    else:
        decision = "ALLOW_APPLY"
        reason = "Stale connector release-gate artifact candidate count is within threshold."
        recommended_command = (
