#!/usr/bin/env python3
"""Apply baseline command-alias artifact cleanup only when policy gate allows."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply baseline command-alias artifact cleanup only when policy allows."
    )
    parser.add_argument(
        "--artifact-dir",
...
def main() -> int:
    ...
    cleanup_module = _load_module("cleanup_sales_baseline_command_aliases_artifacts.py")
    policy_module = _load_module("evaluate_sales_baseline_command_aliases_artifact_cleanup_policy.py")
