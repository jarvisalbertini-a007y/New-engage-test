#!/usr/bin/env python3
"""Evaluate policy gates for event-root backfill artifact cleanup apply-mode."""
...
ALLOW_APPLY_ENV_VAR = "BACKFILL_ARTIFACT_CLEANUP_ALLOW_APPLY"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate whether event-root backfill artifact cleanup may run in apply mode."
        )
    )
    parser.add_argument(
        "--artifact-dir",
...
def evaluate_policy_from_summary(
    ...
    *,
    ...
    if candidate_count <= 0:
        decision = "SKIP_APPLY"
        reason = "No stale event-root backfill artifact fixtures detected."
        recommended_command = None
    elif candidate_count > max_apply_candidates:
        decision = "ACTION_REQUIRED"
        reason = (
            "Candidate count exceeds unattended cleanup threshold; tighten retention "
        ...
        recommended_command = (
            ...
            "verify:telemetry:event-root:backfill:artifact:cleanup:apply:guarded"
        )
    else:
        decision = "ALLOW_APPLY"
        reason = "Stale fixture candidate count is within unattended cleanup threshold."
        recommended_command = (
