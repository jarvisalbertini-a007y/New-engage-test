...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "run_sales_runtime_prereqs_artifact_cleanup_guarded_apply.py"
)


def _load_script_module():
    spec = importlib.util.spec_from_file_location(
        "run_sales_runtime_prereqs_artifact_cleanup_guarded_apply",
        SCRIPT_PATH,
    )
...
def _build_args(artifact_dir: Path, max_apply_candidates: int = 20):
    ...
    _Args.prefix = "sales_runtime_prereqs"
    _Args.keep_days = 30
...
def test_runtime_prereqs_guarded_apply_result_returns_nonzero_for_action_required():
...
def test_runtime_prereqs_guarded_apply_skips_when_no_candidates():
    module = _load_script_module()
    with tempfile.TemporaryDirectory() as tmp:
        artifact_dir = Path(tmp)
        _write_artifact(artifact_dir / "sales_runtime_prereqs_recent.json", datetime.now(timezone.utc))

        original_parse = module.parse_args
        original_env = os.environ.get("RUNTIME_PREREQS_ARTIFACT_CLEANUP_ALLOW_APPLY")
        try:
            module.parse_args = lambda: _build_args(artifact_dir)
            os.environ.pop("RUNTIME_PREREQS_ARTIFACT_CLEANUP_ALLOW_APPLY", None)
            ...
            if original_env is None:
                ...
                os.environ["RUNTIME_PREREQS_ARTIFACT_CLEANUP_ALLOW_APPLY"] = original_env

        assert exit_code == 0


def test_runtime_prereqs_guarded_apply_executes_when_policy_allows_apply():
    module = _load_script_module()
    with tempfile.TemporaryDirectory() as tmp:
        artifact_dir = Path(tmp)
        now = datetime.now(timezone.utc)
        recent_path = artifact_dir / "sales_runtime_prereqs_recent.json"
        ...
        try:
            ...
            os.environ["RUNTIME_PREREQS_ARTIFACT_CLEANUP_ALLOW_APPLY"] = "true"
            exit_code = module.main()
        finally:
            module.parse_args = original_parse
            if original_env is None:
