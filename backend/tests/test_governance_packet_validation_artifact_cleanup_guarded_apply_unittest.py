...
def _load_script_module():
    spec = importlib.util.spec_from_file_location(
        "run_governance_packet_validation_artifact_cleanup_guarded_apply",
        SCRIPT_PATH,
    )
...
def _build_args(artifact_dir: Path, max_apply_candidates: int = 20):
    ...
    _Args.prefix = "governance_packet_validation_fixture"
    _Args.keep_days = 30
...
def test_guarded_apply_skips_when_no_candidates():
    module = _load_script_module()
    with tempfile.TemporaryDirectory() as tmp:
        ...
        original_env = os.environ.get("GOVERNANCE_PACKET_ARTIFACT_CLEANUP_ALLOW_APPLY")
        try:
            module.parse_args = lambda: _build_args(artifact_dir)
            os.environ.pop("GOVERNANCE_PACKET_ARTIFACT_CLEANUP_ALLOW_APPLY", None)
            ...
            if original_env is None:
                ...
                os.environ["GOVERNANCE_PACKET_ARTIFACT_CLEANUP_ALLOW_APPLY"] = original_env
...
def test_guarded_apply_executes_when_policy_allows_apply():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        recent_path = artifact_dir / "governance_packet_validation_fixture_recent.json"
        stale_path = artifact_dir / "governance_packet_validation_fixture_stale.json"
        _write_valid_artifact(recent_path, now)
        ...
        try:
            ...
            os.environ["GOVERNANCE_PACKET_ARTIFACT_CLEANUP_ALLOW_APPLY"] = "true"
            exit_code = module.main()
        finally:
            module.parse_args = original_parse
            if original_env is None:
