...
def test_orchestration_slo_gate_smoke_hold_then_proceed(monkeypatch):
    ...
    client = _build_client(monkeypatch, _FakeDb(telemetry_docs))
    query = (
        "/api/integrations/integrations/telemetry/slo-gates?"
        "days=30&limit=1000&max_error_rate_pct=100&min_schema_v2_pct=90"
        "&min_schema_v2_sample_count=1&max_orchestration_attempt_error_count=1"
        "&max_orchestration_attempt_skipped_count=2"
    )

    hold_response = client.get(query)
    assert hold_response.status_code == 200
    hold_payload = hold_response.json()
    assert hold_payload["decision"] == "HOLD"
    assert hold_payload["gates"]["orchestrationAttemptErrorPassed"] is False
    assert hold_payload["gates"]["orchestrationAttemptSkippedPassed"] is False
    assert hold_payload["orchestrationAudit"]["observedAttemptErrorCount"] == 2
    assert hold_payload["orchestrationAudit"]["observedAttemptSkippedCount"] == 3
    assert hold_payload["orchestrationAudit"]["maxAttemptErrorCountThreshold"] == 1
    assert hold_payload["orchestrationAudit"]["maxAttemptSkippedCountThreshold"] == 2

    telemetry_docs[0]["payload"]["attempt_error_count"] = 1
    telemetry_docs[0]["payload"]["attempt_skipped_count"] = 2

    proceed_response = client.get(query)
    assert proceed_response.status_code == 200
    proceed_payload = proceed_response.json()
    assert proceed_payload["decision"] == "PROCEED"
    assert proceed_payload["gates"]["orchestrationAttemptErrorPassed"] is True
    assert proceed_payload["gates"]["orchestrationAttemptSkippedPassed"] is True
    assert proceed_payload["orchestrationAudit"]["observedAttemptErrorCount"] == 1
    assert proceed_payload["orchestrationAudit"]["observedAttemptSkippedCount"] == 2
